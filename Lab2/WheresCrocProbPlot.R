...
#' @export
runWheresCroc=function(makeMoves,showCroc=F,pause=1, doPlot=T) {
  ...
  while (!is.na(positions[1])) {
    ...
    if (doPlot) {
      plotGameboard(points,edges,move,positions,showCroc, moveInfo$mem$f)
    }
    ...
    for (m in moveInfo$moves) {
      if (m==0) {
        if (positions[1]==positions[4]) {
          ...
          makeMoves(moveInfo,readings,positions[2:4],edges,probs)
...
#' @export
plotGameboard=function(points,edges,move,positions,showCroc, f) {
  plot(points,pch=21,col="gray95",cex=0.1,xlab="X",ylab="Y",main=paste("Where's Croc - Move",move))
  xFrom=points[edges[,1],1]
  yFrom=points[edges[,1],2]
  xTo=points[edges[,2],1]
  yTo=points[edges[,2],2]
  segments(xFrom,yFrom,xTo,yTo, col="gray95")

  if (is.null(f)) f = rep(1/40, length=40)

  fonts = rep(3, length=40)
  colors = matrix(rep(0, length=2*40), nrow=40)
  for (i in 1:40) {
    color = NULL

    if (i == positions[2] || i == positions[3]) {
      color = "goldenrod1"
      border = "goldenrod4"
      fonts[i] = 2
    }
    ...
    if (is.null(color)) {
      ...
      color = sprintf("gray%d", as.integer(grayscale))
      border = sprintf("gray%d", as.integer(max(0, grayscale - 20)))
    }

    points(points[i, 1], points[i, 2], col=border, bg=color, pch=21, cex=probSize(f, i))
    colors[i, ] = c(color, border)
  }

  text(points[,1]+.4, points[,2], labels=as.character(1:40), col=colors[, 2], font=fonts)
}
